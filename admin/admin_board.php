...
/***************************************************************************
    ...
    $Id: admin_board.php,v 1.23 2000/11/29 05:28:27 thefinn Exp $
	...
	case 'setoptions':
		if($submit) {
		   ...
		   if ($row[total] != 0) {
		      ...
		      $sql = "UPDATE config SET sitename = '$name', allow_html = '$html', allow_bbcode = '$bb', allow_sig = '$sig', hot_threshold = $hot, posts_per_page = $ppp, topics_per_page = $tpp, allow_theme_create = 0, override_themes = $override_themes, allow_namechange = $allow_name_change, email_from = '$from', email_sig = '$esig' WHERE selected = 1";
		      $result = mysql_query($sql, $db);
		   } else {
		      // have to do an insert..
		      $sql = "INSERT INTO config (sitename, allow_html, allow_bbcode, allow_sig, hot_threshold, posts_per_page, topics_per_page, allow_theme_create, override_themes, allow_namechange, email_from, email_sig, selected) ";
		      $sql .= "VALUES ('$name', $html, $bb, $sig, $hot, $ppp, $tpp, 0, $override_themes, $allow_name_change, '$from', '$esig', 1)";
		      $result = mysql_query($sql, $db);
		   }
		   if (!$result) {
		      echo mysql_error() . "<br>";
		   ...
		   echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><B>Forum Settings Updated.</B></font></td>";
...
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
	...
	<TD><INPUT TYPE="TEXT" NAME="name" SIZE="30" MAXLENGTH="100" VALUE="<?php echo $sitename?>"></TD>
</TR>
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
...
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
	...
	<TD><TEXTAREA NAME="esig" ROWS="5" COLS="20"><?php echo $email_sig?></TEXTAREA></TD>
</TR>
...
/*		     
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
        <TD><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Allow Users to create Themes:</FONT></TD>
        <TD><INPUT TYPE="RADIO" NAME="allow_themes" VALUE="1" <?php echo $theme_yes?>> Yes <INPUT TYPE="RADIO" NAME="allow_themes" VALUE="0" <?php echo $theme_no?>> No</TD>
</TR>
*/
?>		     
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
...
<?php
	...
	case 'headermetafooter':
		...
		echo "<TABLE width=\"95%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\" bordercolor=\"$table_bgcolor\">";
		echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><B>Data Added.</B></font></td>";
		echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
		echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb\">here</a> to return to the forum index.</font><P><BR><P></TD>";
		echo "</TR></table></TD></TR></TABLE>";
...
<TR BGCOLOR="<?php echo $color1?>" ALIGN="LEFT">
	...
	<TD ALIGN="CENTER"><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Max. Posts</FONT></TD>
        <TD ALIGN="CENTER"><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Image</FONT></TD>
	<TD ALIGN="CENTER"><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Special Rank</FONT></TD>
	<TD ALIGN="CENTER"><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Add</FONT></TD>
</TR>
