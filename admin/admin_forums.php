...
/***************************************************************************
    ...
    $Id: admin_forums.php,v 1.30 2000/11/23 08:00:59 thefinn Exp $
 ***************************************************************************/
...
switch($mode) {
 case 'editforum':
   if($save) {
	 ...
	 /* 
	  This isn't even close to secure so don't tell anyone how it works :) 
	  I'll write a real 2 way encryption algo. for the next release  
	  */
	 $pass = base64_encode($forum_pass);
	 
	 if($type == 1) 
	   $sql = "UPDATE forums SET forum_name = '$name', forum_desc = '$desc', forum_type = '$type', cat_id = '$cat', forum_pass = '$pass', forum_access = '$forum_access' WHERE forum_id = '$forum'";
	 else 
	 ...
	 echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><B>Forum Updated.</B></font></td>";
	 if($mod_not_removed) 
	   echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><b>However the selected moderator(s) have not be removed because if they had been there would no longer be any moderators on this forum.</b></font></td>";
		
	 echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
	 echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb\">here</a> to return to the forum index.</font><P><BR><P></TD>";
	 ...
	 echo "<TABLE width=\"95%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\" bordercolor=\"$table_bgcolor\">";
	 echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><B>Forum Removed.</B></font></td>";
	 echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
	 echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><P><BR>&nbsp;&nbsp;Forum removed from database along with all its posts.<P>Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb\">here</a> to return to the forum index.</font><P><BR><P></TD>";
	 echo "</TR></table></TD></TR></TABLE>";
      }
   }
   if($submit && !$save) {
...
<?php
	$forum_pass = base64_decode($myrow[forum_pass]);
?>
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
	<TD><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Password (If Private):</FONT></TD>
	<TD><INPUT TYPE="TEXT" NAME="forum_pass" SIZE="25" MAXLENGTH="30" VALUE="<?php echo $forum_pass?>"></TD>
</TR>
<TR BGCOLOR="<?php echo $color1?>" ALIGN="LEFT">
 ...
 case 'remcat':
   if($submit) {
	...
	die("Error Deleteing Category<BR>".mysql_error($db));
      echo "<TABLE width=\"95%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\" bordercolor=\"$table_bgcolor\">";
      echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><B>Category Created.</B></font></td>";
      echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
      echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb\">here</a> to return to the forum index.</font><P><BR><P></TD>";
      echo "</TR></table></TD></TR></TABLE>";      
...
<?
 ...
 case 'addforum':
   if($submit) {
	...
	die("You did not fill out all the parts of the form, please go back and do so.");
      $desc = str_replace("\n", "<BR>", $desc);
      $desc = addslashes($desc);
      $name = addslashes($name);
      if($type == 1) {
	 $forum_pass = base64_encode($forum_pass);
	 $sql = "INSERT INTO forums (forum_name, forum_desc, forum_access, cat_id, forum_type, forum_pass) VALUES ('$name', '$desc', '$forum_access', '$cat', '$type', '$forum_pass')";
      }
      else
	$sql = "INSERT INTO forums (forum_name, forum_desc, forum_access, cat_id) VALUES ('$name', '$desc', '$forum_access', '$cat')";
	...
	die("An Error Occurred<HR>Could not contact the database. Please check your config file.<BR>".mysql_error()."<BR>$sql");
	 ...
	 if(!mysql_query($user_query, $db))
      ...
      echo "<TABLE width=\"95%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\" bordercolor=\"$table_bgcolor\">";
      echo "<tr><td align=\"center\" width=\"100%\" bgcolor=\"$color1\"><font face=\"$FontFace\" size=\"$FontSize2\" color=\"$FontColor\"><B>Forum Created.</B></font></td>";
      echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
      echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$FontColor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin/index.$phpEx\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb/viewforum.$phpEx?forum=$forum\">here</a> to view the forum  you just created.</font><P><BR><P></TD>";
      echo "</TR></table></TD></TR></TABLE>";
   }
   else {
...
<TR BGCOLOR="<?php echo $color2?>" ALIGN="LEFT">
        <TD><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>">Password (If Private):</FONT></TD>
        <TD><INPUT TYPE="TEXT" NAME="forum_pass" SIZE="25" MAXLENGTH="30"></TD>
</TR>
<TR BGCOLOR="<?php echo $color1?>" ALIGN="LEFT">
	<TD ALIGN="CENTER" COLSPAN="2">
